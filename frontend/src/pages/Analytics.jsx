...
const Analytics = () => {
  ...
  const calculateAnalyticsData = () => {
    ...
    const filteredServedOrders = filteredOrders.filter(order => order.status === 'served');

    // Calculate metrics
    const totalRevenue = filteredServedOrders.reduce((sum, order) => sum + (order.totalPrice || order.total || 0), 0);
    const totalOrders = filteredOrders.length;
    const averageOrderValue = filteredServedOrders.length > 0 ? totalRevenue / filteredServedOrders.length : 0;
    ...
    const totalCategoryRevenue = Object.values(categoryRevenue).reduce((sum, revenue) => sum + revenue, 0);
    const categories = Object.entries(categoryRevenue).map(([name, revenue]) => ({
      name,
      revenue,
    ...
    const orderStatus = {
      served: filteredOrders.filter(order => order.status === 'served').length,
    ...
    filteredOrders.forEach(order => {
      order.menuItems?.forEach(item => {
        ...
        itemPopularity[itemName] = itemPopularity[itemName] || { orders: 0, revenue: 0 };
        itemPopularity[itemName].orders += item.quantity || 1;
        itemPopularity[itemName].revenue += (item.quantity || 1) * (item.price || 0);
      });
    });
    ...
    const totalCustomers = new Set(filteredOrders.map(order => order.userId?._id)).size;
    ...
    return {
      ...
      customers: {
        ...
        feedback: [
          { rating: 5, count: Math.floor(totalCustomers * 0.67), percentage: 67 },
          { rating: 4, count: Math.floor(totalCustomers * 0.25), percentage: 25 },
          { rating: 3, count: Math.floor(totalCustomers * 0.05), percentage: 5 },
          { rating: 2, count: Math.floor(totalCustomers * 0.02), percentage: 2 },
  ...
  const generateDailyData = (orders) => {
    ...
    orders.forEach(order => {
      ...
      dailyData[dayKey] = (dailyData[dayKey] || 0) + (order.totalPrice || order.total || 0);
    });

    return days.map(day => ({
      date: day,
      revenue: dailyData[day] || 0,
      orders: orders.filter(order => days[new Date(order.createdAt).getDay()] === day).length
    }));
  };
          ...
          <div className="col-lg-8 mb-4">
            <div className="card shadow">
              <div className="card-header py-3">
                <h6 className="m-0 font-weight-bold text-primary">Revenue Trend - {timeRange.charAt(0).toUpperCase() + timeRange.slice(1)}</h6>
              </div>
              <div className="card-body">
          ...
          <div className="col-lg-3 col-md-6 mb-4">
            <div className="card bg-success text-white">
              <div className="card-body">
                <div className="d-flex justify-content-between">
                  <div>
                    <h4 className="mb-0">{analyticsData.orders.status.served}</h4>
                    <small>Served</small>
                  </div>
                  ...
                  <div key={index} className="mb-3">
                    ...
                    <div className="progress" style={{ height: '10px' }}>
                      <div
                        ...
                        style={{ width: `${(hour.orders / Math.max(...analyticsData.orders.peakHours.map(h => h.orders))) * 100}%` }}
                      ></div>
                    </div>
                  </div>
                ))}
